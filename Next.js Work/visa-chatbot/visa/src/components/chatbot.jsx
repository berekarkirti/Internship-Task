...
export default function Chatbot() 
{
  ...
  const isRelatedToContext = (currentQuery, recentMessages) => 
  {
    ...
    const visaKeywords = ["visa", "student", "tourist", "documents", "apply", "eligibility", "process", "disney"];
  ...
  const getSuggestedQuestions = (query, responseText) => 
  {
    ...
    if (queryLower.includes("disney") || queryLower.includes("tourist")) 
    {
      return [
        ...
        "What documents are needed for a Disney trip?",
    ...
    {
      return [
        "What are ties to India?",
        "Can I get a sponsor for funds?",
    ...
    {
      return [
        "Can you explain B-2 visa requirements?",
        "How to book a visa interview?",
  ...
  const handleSendMessage = async (e, suggestedQuestion = null) => 
  {
    ...
    try 
    {
      ...
      const recentMessages = messages.slice(-6).map((msg) => ({ role: msg.sender === "user" ? "user" : "assistant", content: msg.text, }));
  ...
  return (
    <div className="fixed bottom-20 right-6 w-80 sm:w-[420px] max-w-[90vw] h-[560px] bg-white rounded-2xl shadow-2xl flex flex-col animate-slide-up overflow-hidden">
      ...
      <div className="flex-1 p-5 bg-gray-50 overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-teal-300 scrollbar-track-gray-100">
        {messages.length === 0 && (<div className="text-center text-gray-600 text-sm mt-4">Ask about visa requirements, processes, or documents!</div>)}
          ...
          <div className="flex justify-start">
            <div className="max-w-[75%] p-3 rounded-xl bg-white text-gray-600 text-sm shadow-sm border border-gray-200">
              <span className="inline-flex items-center space-x-1">
                <span className="animate-pulse">{detectLanguage(input) === "Gujarati" ? "ટાઇપિંગ" : "Typing"}</span>
      ...
      <div className="px-5 py-4 bg-white border-t border-gray-200">
        <div className="flex items-center space-x-3">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === "Enter" && handleSendMessage(e)} placeholder="Write your doubt here..." className="flex-1 px-4 py-2.5 text-sm bg-gray-100 border border-gray-200 rounded-full focus:outline-none focus:border-teal-500 focus:bg-white transition-all duration-200 placeholder-gray-600" disabled={isLoading} />
