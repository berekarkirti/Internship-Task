...
//   // States for "Update Product" form
...
//     if (!addName.trim() || addImages.length === 0 || !addPrice.trim()) {
...
//         `http://localhost:1337/api/products/${updateId}`,
//         {
//           data: {
//             name: updateName,
//             price: updatePrice,
...
//       <button onClick={() => setShowAddForm(!showAddForm)} className="bg-teal-600 text-white py-3 px-6 rounded-md mb-4 mt-8 hover:bg-teal-700 transition duration-300 shadow-md">
//         {showAddForm ? "Close Add Form" : "Add Product"}
//       </button>
...
//             className="max-w-md mx-auto bg-white p-6 rounded-xl shadow-lg space-y-6 text-black border border-gray-200"
//             encType="multipart/form-data"
//           >
//             <label htmlFor="addName" className="block text-gray-700 font-medium">Name:</label>
//             <input
//               type="text"
//               id="addName"
//               name="name"
...
//               className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
...
//             <label htmlFor="addImage" className="block text-gray-700 font-medium">Image:</label>
//             <input
//               type="file"
//               id="addImage"
//               name="image"
...
//                 setAddImages(files);
//                 setAddImagePreviews(files.map((file) => URL.createObjectURL(file)));
//               }}
//               className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
//               required
//             />
...
//                     src={preview}
//                     alt="Preview"
//                     className="w-24 h-24 object-cover rounded-md border border-gray-200 shadow-sm mx-auto"
//                   />
//                   <button
//                     type="button"
//                     onClick={() => {
//                       const newPreviews = addImagePreviews.filter((_, i) => i !== index);
//                       const newFiles = addImages.filter((_, i) => i !== index);
//                       setAddImagePreviews(newPreviews);
//                       setAddImages(newFiles);
//                     }}
//                     className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
//                   >
//                     ×
//                   </button>
//                 </div>
...
//             <button
//               type="submit"
//               className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 px-6 rounded-md transition duration-300 shadow-md"
//             >
//               Submit
//             </button>
//           </form>
...
//       {showUpdateForm && (
//         <div className="my-8">
//           <h2 className="text-2xl mb-4 font-semibold text-white">Update Product</h2>
...
//             <input type="number" id="addPrice" name="price" value={updatePrice} onChange={(e) => setUpdatePrice(e.target.value)} className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800" placeholder="Enter product price" required />
...
//                 setUpdateImagePreviews(files.map((file) => URL.createObjectURL(file)));
//               }}
//               className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
//             />
//             <div className="flex flex-wrap gap-4 mt-4">
...
//                   <img src={preview} alt="Preview" className="w-24 h-24 object-cover rounded-md border border-gray-200 shadow-sm mx-auto" />
//                 </div>
//               ))}
//             </div>
...
//         {loading ? (<p className="text-gray-500">Loading products...</p>) : products.length > 0 ?
//           (
//             products.map((item) =>
//             (
...
const ProductComponent = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [addImagePreviews, setAddImagePreviews] = useState({ en: [], gu: [], pa: [], ko: [] });
  const [updateImagePreviews, setUpdateImagePreviews] = useState([]);
  ...
  const [showAddForm, setShowAddForm] = useState(false);
  const [addNames, setAddNames] = useState({ en: '', gu: '', pa: '', ko: '' });
  const [addPrice, setAddPrice] = useState('');
  const [addImages, setAddImages] = useState({ en: [], gu: [], pa: [], ko: [] });
  ...
  const fetchProducts = async () => {
    try {
      const productsData = await getProducts({ locale: selectedLocale });
      console.log(`Products for ${selectedLocale}:`, productsData);
      setProducts(productsData || []);
    } catch (error) {
      console.error('Error fetching products:', error);
    } finally {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      // Create English product (base)
      const englishProductResponse = await axios.post(
        'http://localhost:1337/api/products',
        {
          data: {
            name: addNames.en,
            price: addPrice || 0,
            images: uploadedImageIds.en,
            locale: 'en',
          },
        },
        {
      ...
      const englishProductId = englishProductResponse.data.data.documentId;
          ...
          await axios.post(
            'http://localhost:1337/api/products',
            {
              data: {
                ...
                locale: localeCode,
                localizations: [englishProductId], // Link to English product
              },
            },
            {
              headers: {
      ...
      console.log('Product created successfully with translations');
      setAddNames({ en: '', gu: '', pa: '', ko: '' });
      setAddPrice('');
      setAddImages({ en: [], gu: [], pa: [], ko: [] });
      setAddImagePreviews({ en: [], gu: [], pa: [], ko: [] });
      setShowAddForm(false);
      fetchProducts();
      alert('Product created successfully with translations!');
    } catch (error) {
      console.error('Error:', error.response?.data || error.message);
      alert('An error occurred while creating the product. Please try again.');
    }
  ...
  return (
    <div className="text-center p-8 rounded-xl shadow-lg">
      ...
      <button
        ...
        className="bg-teal-600 text-white py-3 px-6 rounded-md mb-4 mt-8 hover:bg-teal-700 transition duration-300 shadow-md"
      >
        {showAddForm ? 'Close Add Form' : 'Add Product'}
      </button>
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                id="addNameEn"
                value={addNames.en}
                onChange={(e) => setAddNames({ ...addNames, en: e.target.value })}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
              ...
              <input
                ...
                onChange={(e) => {
                  const files = Array.from(e.target.files);
                  setAddImages({ ...addImages, en: files });
                  setAddImagePreviews({ ...addImagePreviews, en: files.map((file) => URL.createObjectURL(file)) });
                }}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
                required
              />
                ...
                {addImagePreviews.en.map((preview, index) => (
                  <div key={index} className="relative group w-1/2 p-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newPreviews = addImagePreviews.en.filter((_, i) => i !== index);
                        const newFiles = addImages.en.filter((_, i) => i !== index);
                        setAddImagePreviews({ ...addImagePreviews, en: newPreviews });
                        setAddImages({ ...addImages, en: newFiles });
                      }}
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                    >
                      ×
            ...
            <div>
              ...
              <input
                ...
                id="addNameGu"
                value={addNames.gu}
                onChange={(e) => setAddNames({ ...addNames, gu: e.target.value })}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
                placeholder="Enter product name in Gujarati"
              />
              ...
              <input
                ...
                onChange={(e) => {
                  const files = Array.from(e.target.files);
                  setAddImages({ ...addImages, gu: files });
                  setAddImagePreviews({ ...addImagePreviews, gu: files.map((file) => URL.createObjectURL(file)) });
                }}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
              />
              <div className="flex flex-wrap gap-4 mt-4">
                {addImagePreviews.gu.map((preview, index) => (
                  <div key={index} className="relative group w-1/2 p-2">
                    <img
                      src={preview}
                      alt="Preview"
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newPreviews = addImagePreviews.gu.filter((_, i) => i !== index);
                        const newFiles = addImages.gu.filter((_, i) => i !== index);
                        setAddImagePreviews({ ...addImagePreviews, gu: newPreviews });
                        setAddImages({ ...addImages, gu: newFiles });
                      }}
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                    >
                      ×
            ...
            <div>
              ...
              <input
                ...
                id="addNamePa"
                value={addNames.pa}
                onChange={(e) => setAddNames({ ...addNames, pa: e.target.value })}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
                placeholder="Enter product name in Punjabi"
              />
              ...
              <input
                ...
                onChange={(e) => {
                  const files = Array.from(e.target.files);
                  setAddImages({ ...addImages, pa: files });
                  setAddImagePreviews({ ...addImagePreviews, pa: files.map((file) => URL.createObjectURL(file)) });
                }}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
              />
              <div className="flex flex-wrap gap-4 mt-4">
                {addImagePreviews.pa.map((preview, index) => (
                  <div key={index} className="relative group w-1/2 p-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newPreviews = addImagePreviews.pa.filter((_, i) => i !== index);
                        const newFiles = addImages.pa.filter((_, i) => i !== index);
                        setAddImagePreviews({ ...addImagePreviews, pa: newPreviews });
                        setAddImages({ ...addImages, pa: newFiles });
                      }}
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                    >
                      ×
            ...
            <div>
              ...
              <input
                ...
                id="addNameKo"
                value={addNames.ko}
                onChange={(e) => setAddNames({ ...addNames, ko: e.target.value })}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
              ...
              <input
                ...
                onChange={(e) => {
                  const files = Array.from(e.target.files);
                  setAddImages({ ...addImages, ko: files });
                  setAddImagePreviews({ ...addImagePreviews, ko: files.map((file) => URL.createObjectURL(file)) });
                }}
                className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-800"
              />
              <div className="flex flex-wrap gap-4 mt-4">
                {addImagePreviews.ko.map((preview, index) => (
                  <div key={index} className="relative group w-1/2 p-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newPreviews = addImagePreviews.ko.filter((_, i) => i !== index);
                        const newFiles = addImages.ko.filter((_, i) => i !== index);
                        setAddImagePreviews({ ...addImagePreviews, ko: newPreviews });
                        setAddImages({ ...addImages, ko: newFiles });
                      }}
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                    >
                      ×
      ...
      <div className="flex justify-center gap-6 flex-wrap shadow-5xl">
        {loading ? (
          ...
          products.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl shadow-md hover:shadow-lg transition duration-300 w-1/3 p-6 space-y-4"
            >
              <div className="flex flex-wrap justify-center gap-4 mb-4">
                {item.images && Array.isArray(item.images) ? (
                  item.images.map((img, imgIndex) => {
                    ...
                    return (
                      <div key={imgIndex} className="w-1/3 p-2">
                        <img
                          src={`http://localhost:1337${finalUrl}`}
              ...
              <div className="space-x-4">
                <button
                  ...
                  className="bg-teal-600 text-white py-3 px-6 rounded-md hover:bg-teal-700 transition duration-300 shadow-md"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(item.documentId)}
                  className="bg-red-500 text-white py-3 px-6 rounded-md hover:bg-red-600 transition duration-300 shadow-md"
                >
                  Delete
                </button>
              </div>
